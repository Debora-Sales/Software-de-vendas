...
class JanelaRelatorios(ctk.CTkToplevel):
    ...
    def _setup_ui_historico(self):
        ...
        header.pack(fill="x", padx=10, pady=5)
        ctk.CTkLabel(header, text="ID", width=60, font=("", 12, "bold")).pack(side="left", padx=10)
        ctk.CTkLabel(header, text="Vendedor", width=200, anchor="w", font=("", 12, "bold")).pack(side="left")
        ctk.CTkLabel(header, text="Cliente", width=200, anchor="w", font=("", 12, "bold")).pack(side="left")
        ctk.CTkLabel(header, text="Data/Hora", width=150, font=("", 12, "bold")).pack(side="left")
        ctk.CTkLabel(header, text="Valor Total", width=150, font=("", 12, "bold")).pack(side="left")
    ...
    def carregar_dados(self):
        ...
        for id_v, vend, cli, data, total in dados_historico:
            f = ctk.CTkFrame(self.scroll_historico)
            f.pack(fill="x", pady=2)
            ctk.CTkLabel(f, text=str(id_v), width=60).pack(side="left", padx=10)
            ctk.CTkLabel(f, text=vend, width=200, anchor="w").pack(side="left")
            ctk.CTkLabel(f, text=cli, width=200, anchor="w").pack(side="left")
            ctk.CTkLabel(f, text=data, width=150).pack(side="left")
            ctk.CTkLabel(f, text=f"R$ {total:,.2f}", width=150, text_color="orange", font=("", 12, "bold")).pack(side="left")
            
        # Feedback visual final
        if not dados_historico:
            lbl_vazio = ctk.CTkLabel(self.scroll_historico, text="Nenhuma venda encontrada.\nRealize uma venda no PDV primeiro.", text_color="gray")
            lbl_vazio.pack(pady=50)
        
        messagebox.showinfo("Atualização", "Dados sincronizados com o banco de dados.")